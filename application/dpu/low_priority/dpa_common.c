/* DPA flow demo: shared helpers. */

#include "dpa_common.h"

DOCA_LOG_REGISTER(DPA_COMMON);

/* Set by DPACC at link-time; name must match DPACC --app-name. */
extern struct doca_dpa_app *dpa_sample_app;

/* ARGP callback for --device. */
...
/* Open a DPA-capable DOCA device (or pick one if device_name == NOT_SET). */
static doca_error_t open_dpa_device(const char *device_name, struct doca_dev **doca_device)
{
	...
	/* Find a DPA-capable device, optionally matching IB device name. */
	for (i = 0; i < nb_devs; i++) {
