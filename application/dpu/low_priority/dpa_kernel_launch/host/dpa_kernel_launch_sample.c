...
static const uint32_t k_n_iters_in_kernel = 300000;
...
static bool getenv_u32_checked(const char *name, uint32_t *out_value)
{
...
static uint32_t getenv_u32_or_default(const char *name, uint32_t default_value)
{
	...
	if (getenv_u32_checked(name, &parsed_value))
		return parsed_value;
	return default_value;
}

static uint32_t getenv_u32_first_of(const char *const *names,
					 size_t count,
					 uint32_t default_value)
{
	uint32_t parsed_value;
	for (size_t i = 0; i < count; ++i) {
		if (getenv_u32_checked(names[i], &parsed_value))
			return parsed_value;
	}
	return default_value;
}
...
doca_error_t kernel_launch(struct dpa_resources *resources)
{
	...
	const char *const task_envs[] = {"DPA_DEMO_TASKS", "DPA_DEMO_TOTAL_TASKS"};
	const uint32_t m_tasks = getenv_u32_first_of(task_envs,
			sizeof(task_envs) / sizeof(task_envs[0]),
			k_m_tasks);
	const char *const iter_envs[] = {"DPA_DEMO_KERNEL_ITERS", "DPA_DEMO_ITERS"};
	const uint32_t n_iters_in_kernel = getenv_u32_first_of(iter_envs,
			sizeof(iter_envs) / sizeof(iter_envs[0]),
			k_n_iters_in_kernel);
	uint32_t sleep_min_ms = getenv_u32_or_default("DPA_DEMO_SLEEP_MIN_MS", k_sleep_min_ms);
	uint32_t sleep_max_ms = getenv_u32_or_default("DPA_DEMO_SLEEP_MAX_MS", k_sleep_max_ms);
	uint32_t fixed_sleep_ms;
	if (getenv_u32_checked("DPA_DEMO_INTER_TASK_SLEEP_MS", &fixed_sleep_ms)) {
		sleep_min_ms = fixed_sleep_ms;
		sleep_max_ms = fixed_sleep_ms;
	}
	const char *const inflight_envs[] = {"DPA_DEMO_MAX_INFLIGHT", "DPA_DEMO_INFLIGHT"};
	uint32_t max_inflight = getenv_u32_first_of(inflight_envs,
			sizeof(inflight_envs) / sizeof(inflight_envs[0]),
			k_max_inflight_default);
	const uint32_t gate_batch = getenv_u32_or_default("DPA_DEMO_GATE_BATCH", 0);
	const uint32_t gate_hold_ms = getenv_u32_or_default("DPA_DEMO_GATE_HOLD_MS", 0);
	const uint32_t gate_idle_ms = getenv_u32_or_default("DPA_DEMO_GATE_IDLE_MS", 0);
	const uint32_t gate_drain = getenv_u32_or_default("DPA_DEMO_GATE_DRAIN", gate_batch != 0 ? 1 : 0);
	const uint32_t gate_log_ms = getenv_u32_or_default("DPA_DEMO_GATE_LOG_MS", gate_batch != 0 ? 500 : 0);
	const uint32_t gate_verbose = getenv_u32_or_default("DPA_DEMO_GATE_VERBOSE", gate_batch != 0 ? 1 : 0);
	...
	if (max_inflight == 0)
		max_inflight = 1;
	if (gate_batch != 0 && max_inflight < gate_batch)
		max_inflight = gate_batch;

	if (gate_verbose != 0) {
		fprintf(stderr,
			"[gate] tasks=%u iters=%u inflight=%u gate_batch=%u gate_hold_ms=%u gate_idle_ms=%u gate_drain=%u\n",
			m_tasks,
			n_iters_in_kernel,
			max_inflight,
			gate_batch,
	...
	for (uint32_t task_id = 0; task_id < m_tasks; task_id++) {
		...
		uint64_t wait_thresh = (gate_batch == 0) ? 4 : gate_wait_val;
		...
		if (gate_batch != 0) {
			gate_in_batch++;
			if (gate_in_batch >= gate_batch) {
				if (gate_hold_ms != 0)
					sleep_ms_range(gate_hold_ms, gate_hold_ms);
				if (g_stop_requested) {
					(void)doca_sync_event_update_set(wait_event, UINT64_MAX);
					goto cleanup;
				}
				result = doca_sync_event_update_set(wait_event, gate_wait_val + 1);
				if (result != DOCA_SUCCESS)
					goto cleanup;
				gate_wait_val += 2;
				gate_in_batch = 0;
				if (gate_drain != 0) {
					pthread_mutex_lock(&worker_ctx.mutex);
					uint32_t target_completed = worker_ctx.next_ready;
					uint64_t last_log_ns = 0;
					while (worker_ctx.completed < target_completed &&
					       worker_ctx.worker_result == DOCA_SUCCESS &&
					       !g_stop_requested) {
						uint32_t poll_ms = (gate_log_ms == 0) ? 200 : gate_log_ms;
						struct timespec ts;
						(void)clock_gettime(CLOCK_REALTIME, &ts);
						timespec_add_ms(&ts, poll_ms);
						(void)pthread_cond_timedwait(&worker_ctx.cond, &worker_ctx.mutex, &ts);
						if (gate_verbose != 0 && gate_log_ms != 0) {
							uint64_t now = get_time_ns();
							if (last_log_ns == 0 || now - last_log_ns >= (uint64_t)gate_log_ms * 1000000ULL) {
								fprintf(stderr, "[gate] drain %u/%u\n", worker_ctx.completed, target_completed);
								fflush(stderr);
								last_log_ns = now;
							}
						}
					...
					pthread_mutex_unlock(&worker_ctx.mutex);
					if (gate_idle_ms != 0)
						sleep_ms_range(gate_idle_ms, gate_idle_ms);
				}
			}
		}
	}

	if (gate_batch != 0 && gate_in_batch != 0) {
		if (gate_hold_ms != 0)
			sleep_ms_range(gate_hold_ms, gate_hold_ms);
		if (g_stop_requested) {
			(void)doca_sync_event_update_set(wait_event, UINT64_MAX);
...
cleanup:
	...
	/* No further cleanup labels required beyond this point. */
	if (comp_event != NULL)
