...
void FillRandom(void *buf, size_t size) {
    ...
    for (size_t i = size4 * sizeof(int); i < size; ++i) {
        buf1[i] = rand();
    }
}
...
size_t GetSize(nvinfer1::DataType data_type) {
    ...
    case nvinfer1::DataType::kBOOL:
        return 1;
    default:
        ASSERT(false, "unknown data type(%d)", (int)data_type);
        return 8;
    }
}
...
Tensor::Tensor(size_t size, const nvinfer1::Dims &dims, const std::string &name) : size_(size), dims_(dims), name_(name) {
...
void Tensor::CopyToHostAsync(cudaStream_t stream) {
    CUDART_ASSERT(cudaMemcpyAsync(host_buf_, device_buf_, size_, cudaMemcpyDeviceToHost, stream));
}

void Tensor::CopyToDeviceAsync(cudaStream_t stream) {
    CUDART_ASSERT(cudaMemcpyAsync(device_buf_, host_buf_, size_, cudaMemcpyHostToDevice, stream));
}
...
void Tensor::Load(const std::string &filename) {
    ...
    if (!file.good()) {
        ...
        WARN("[TENSOR] input tensor file %s does not exist, tensor %s will be randoms", filename.c_str(), name_.c_str());
        return;
    }

    std::streamsize size = file.tellg();
    ASSERT(size == (std::streamsize)size_, "file size (%ld Bytes) mismatch tensor size (%ld Bytes)", size, size_);
    INFO("[TENSOR] load %s to tensor %s (%ld Bytes)", filename.c_str(), name_.c_str(), size_);

    file.seekg(0, std::ios::beg);
    ASSERT(file.read((char *)host_buf_, size), "read file %s failed", filename.c_str());
    file.close();
}

void Tensor::Save(const std::string &filename) {
    INFO("[TENSOR] save tensor %s (%ld Bytes) to %s", name_.c_str(), size_, filename.c_str());
