...
int main(int argc, char **argv)
{
    ...
    psync.Sync(2, "Bg race init done");

    if (warmup_tasks > 0 && dpu_up::RunForTaskCount(warmup_tasks) == 0) {
        XERRO("bg race warmup failed");
    }

    psync.Sync(4, "Bg race warmup done");
    psync.Sync(6, "Bg race start");
    const auto start = std::chrono::steady_clock::now();
    const uint64_t completed = dpu_up::RunForTaskCount(race_tasks);
    if (completed != race_tasks) XERRO("bg race task count mismatch");
    ...
    file.close();
    std::printf("RESULT role=bg-race elapsed=%.6f s throughput=%.2f tasks/s\n", elapsed_s, thpt);
    std::fflush(stdout);

    XINFO("[RACE] Bg finished %lu tasks in %.6f s (%.2f tasks/s)", completed, elapsed_s, thpt);
