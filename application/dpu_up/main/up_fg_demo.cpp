...
int main(int argc, char **argv)
{
    ...
    ProcessSync psync;
    psync.Sync(2, "Fg demo init done");

    DemoRunner runner;
    runner.Warmup(warmup_cmds, cmd_us);

    psync.Sync(4, "Fg demo warmup done");
    ...
    file.close();
    std::printf("RESULT role=fg-demo completed=%lu elapsed=%.6f s rate=%.2f cmds/s\n", completed, elapsed_s, rate);
    std::fflush(stdout);

    XINFO("[DEMO] Fg completed %lu cmds in %.6f s (%.2f cmds/s)", completed, elapsed_s, rate);
    return 0;
}
