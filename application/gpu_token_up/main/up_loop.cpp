...
class TokenThrottle {
  public:
    ...
    explicit TokenThrottle(bool enabled)
        : enabled_(enabled),
          quota_unit_(ParsePositiveLong(std::getenv("GPU_TOKEN_UP_TOKEN_QUOTA_UNIT"), 1)),
          window_ms_(ParsePositiveLong(std::getenv("GPU_TOKEN_UP_TOKEN_WINDOW_MS"), 1000)),
          utilization_file_(std::getenv("DREX_UTILIZATION_FILE") != nullptr ? std::getenv("DREX_UTILIZATION_FILE") : "") {
        Refill(clock::now());
    }
    ...
    long utilization() const {
        return utilization_;
    }

    long quota_unit() const {
        return quota_unit_;
    }
    ...
    void Acquire() {
        ...
        while (!g_stop) {
            ...
            if (now >= window_end_ || tokens_ <= 0) {
                if (now < window_end_ && tokens_ <= 0) {
                    std::this_thread::sleep_until(window_end_);
                }
                Refill(clock::now());
                continue;
            }
  ...
  private:
    long ReadUtilization() const {
        if (utilization_file_.empty()) {
            return 100;
        }

        std::ifstream file(utilization_file_);
        long value = 100;
        if (!(file >> value)) {
            return utilization_;
        }
    ...
    void Refill(clock::time_point now) {
        ...
        utilization_ = ReadUtilization();
        tokens_ = std::max<long>(1, utilization_ * quota_unit_);
        window_end_ = now + std::chrono::milliseconds(window_ms_);
    }

    bool enabled_ = false;
    long quota_unit_ = 1;
    long window_ms_ = 1000;
    std::string utilization_file_;
    long utilization_ = 100;
    long tokens_ = 100;
    clock::time_point window_end_ = clock::now();
};
...
int main(int argc, char **argv) {
    ...
    TokenThrottle throttle(ParseEnabled(std::getenv("GPU_TOKEN_UP_TOKEN_THROTTLE")));
    ...
    if (throttle.enabled()) {
        INFO("Token throttle enabled: utilization=%ld quota_unit=%ld", throttle.utilization(), throttle.quota_unit());
    }
