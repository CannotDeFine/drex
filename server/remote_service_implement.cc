...
grpc::Status RemoteServiceImplement::DownloadFile(ServerReader<TaskRequest>* request,
                                                  TaskResult* result,
    ...
    while (request->Read(&task_request)) {
        ...
        if (!task_request.path().empty()) {
            grpc::Status path_status =
                ResolveTaskFilePath(task_request.path(), workspace_root, &target_path);
            if (!path_status.ok()) {
